...
function registrointemporal(){
	...
	mensajesinespacios = $.trim($("#descripcion").val());
	$("#mensajefbn").val(mensajesinespacios);
...
function listmensajes(){
	...
	$.get(url , $("#mensajefbform").serialize() )
		...
		llenaelementoHTML("#numeromensajetw" , numeromensajesTW);
		llenaelementoHTML("#numeromensajefb" , numeromensajesFB);
...
function configmensajeacct(evt){
		...
		url =  $(".now").val() +"index.php/camp/editainformacionmensaje?mensaje="+idmensaje+"&campid="+campid+"&name="+$("#name").val();
...
function generahorariosdisponibles(){
	...
	valorHTML( inputcheckedef , "1");

	

	listhoras = "<div><table class='large-12 columns' >";
	...
	for (var a = 0; a <= 23; a++) {
		...
		listhoras +="<tr class='"+nombreev+"'  ><td><label class='"+fhora+"'>"+hora+" </label> </td><td><label class='"+fhorae+"'> Fijar hora <img width='5%' class='horalistado' id='"+a+"' src='"+urlimg+"'> </label> </td></tr>";	

...
class Mensajerest extends REST_Controller
{
    ...
    function updatemensajebyidandaccount_POST(){
          ...
          if ($logged_in == 1) {
            ...
            $idmensaje = $this->input->post("mensaje");            
            $iduser= $this->session->userdata('idusuario');                        
            ...
            $result= $this->mensajemodel->updatemensajebyidandaccount( $idmensaje , trim($descripcionedit ),  
              trim($name) , trim($descriptioncaption) , $caption , $source , $picture , $link , $lc , $mc , $mic , $jc , $vc , $sc , $dc , $hora_inicioconfig ,  $hora_terminoconfig );
    ...
    function registrotwitter_POST(){  
        ...
        if ($logged_in == 1) {
            ...
            $hora_inicio_msj  = $this->input->post("hora_inicio_msj");
            $hora_termino_msj = $this->input->post("hora_termino_msj");
            ...
            $result  = $this->mensajemodel->registramensajetw($hora_inicio_msj , $hora_termino_msj, $mensaje , $lunes , $martes, $miercoles , $jueves, $viernes , $sabado, $domingo, $campid );
     ...
     function registrointemporalmsjfb_POST(){
        ...
        if ($logged_in == 1) {
            ...
            $iduser= $this->session->userdata('idusuario');
            $descripcion = $this->input->post("mensajefbn");
            $campid = $this->input->post("campid");
            ...
            $result  = $this->mensajemodel->registramensajefb( $iduser , trim($descripcion)  , trim($campid)  , trim($name) , trim($descriptioncaption) , trim($caption)  ,
             trim($source) , trim($picture) , trim($link) ,trim($hinicio) , trim($htermino) ,   $lunes , $martes , $miercoles , $jueves , $viernes , $sabado , $domingo);

...
function twittermensaje(){
    ...
    /*Cuando no hay sessión*/
    if (!$_SESSION["twitter_status"]) {
    
    $twitter  = new TwitterOAuth(_CONSUMER_KEY, _CONSUMER_SECRET);           
    
    $dinamic_url_to_post = base_url()."index.php/socialapi/publicamensajetwittercallback/?twittermsj=".$mydescriptionmsj."&zona=".$zona;
    ...
    header("location:".$twitter_url);
        ...
        $followers_count=  $credenciales->followers_count;
        $friends_count = $credenciales->friends_count;        
        $url =  $credenciales->url;
        $location = $credenciales->location;
        $profile_image_url=  $credenciales->profile_image_url;
...
function trypostfb(){
    ...
    $mensaje = trim($this->input->get("mensaje"));
...
public function zonaatt(){
    ...
    for ($a= 0; $a < count($listmensajes); $a++){ 
        ...
        if ($banderadisponible == 1){
            ...
            if ($listmensajes[$a]["social"]  ==  "F") {
                            ...
                            $mensaje = trim($listmensajes[$a]["descripcion"] ,  "\t\n\r\0\x0B" );
                           ...
                           if($fb_id == 0){                            
                                ...
                                $dinamica="index.php/appqrsocial/fbmensaje?mensaje=".$mensaje."&name=".$name."&descriptioncaption=".$descriptioncaption."&caption=".$caption."&source=".$source."&picture=".$picture."&link=".$link."&format=json";
                                $login =  trim("location:".base_url($dinamica));
                                $loginok = str_replace("\n", " ", $login);
                                header($loginok);
                                ...
                                $basedinamica = "index.php/appqrsocial/postFBnextloggin/?mensaje=".$mensaje."&name=".$name."&descriptioncaption=".$descriptioncaption."&caption=".$caption."&source=".$source."&picture=".$picture."&link=".$link;    
                                $dinamicurl=trim("location:".base_url($basedinamica));                                                                
                                $dinamicurlok = str_replace("\n", " ", $dinamicurl);
                                header($dinamicurlok);                                                    
                ...
                /*Publicar en Twitter*/                
                $mensaje = trim($listmensajes[$a]["descripcion"]);                                
                $dinamic_url_tw = base_url()."index.php/appqrsocial/twittermensaje/?twittermsj=". str_replace("\n", " ",$mensaje)."&zona=".$idzona;
    ...
    if ($flag == 1) {
        ...
        $urlnn ='index.php/appqrsocial/socialuser/?mensaje='.$mensaje."&zona=".$idzona;
        $next = base_url($urlnn ); 
...
function socialuser(){
        ...
        $this->load->view("social/userchoicesocial" , $data);        
